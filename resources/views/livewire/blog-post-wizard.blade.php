<div>
    @if ($isOpen)
        ...
        >
            <!-- Background overlay -->
            <div class="fixed inset-0 bg-gray-950/50 transition-opacity dark:bg-gray-950/75"></div>

            <!-- Modal panel -->
            <div class="flex min-h-full items-center justify-center p-4">
                <div class="relative w-full max-w-3xl rounded-xl bg-white shadow-xl ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10">
                    <!-- Header -->
                    ...
                    <div class="px-6 py-6">
                        @if ($step === 1 || $step === 2)
                            ...
                            <form wire:submit="nextStep">{{ $this->form }}</form>
                        @endif
                        ...
                        @if ($step === 3)
                            <div wire:init="generateBlogPost" class="space-y-4">
                                <div class="mb-4 flex items-center gap-3">
                                    <div class="bg-primary-100 dark:bg-primary-500/10 inline-flex h-10 w-10 items-center justify-center rounded-full">
                                        <svg class="text-primary-600 dark:text-primary-400 h-5 w-5 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ $focus }}</p>
                                    </div>
                                </div>

                                @if ($generatedContent)
                                    <div
                                        wire:stream="generatedContent"
                                        class="prose prose-sm dark:prose-invert max-h-96 max-w-none overflow-y-auto rounded-lg border border-gray-200 p-4 dark:border-gray-700"
                                    >
                        ...
                        @if ($step === 4 && $generatedContent)
                            <div class="space-y-4">
                                ...
                                <div class="prose prose-sm dark:prose-invert max-h-96 max-w-none overflow-y-auto rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                                    {!! \Illuminate\Support\Str::markdown($generatedContent) !!}
                                </div>
                            </div>
                        @endif
                    ...
                    <div class="flex items-center justify-between border-t border-gray-200 px-6 py-4 dark:border-gray-700">
                        ...
                        @elseif ($step === 4)
                            <x-filament::button color="gray" wire:click="regenerate"> Regenerate </x-filament::button>

                            <div class="flex items-center gap-2">
                                <x-filament::button color="gray" wire:click="close"> Cancel </x-filament::button>
                                <x-filament::button wire:click="insertContent"> Insert into Editor </x-filament::button>
                            </div>
                        @endif
                    </div>
                </div>
