                                        ...
                                        Select::make('post_author_id')
                                            ->relationship(
                                                ...
                                                modifyQueryUsing: fn ($query) => $query
                                                    ->selectRaw('*, EXISTS (SELECT 1 FROM user_bios WHERE user_bios.user_id = ' . $query->getModel()->getTable() . '.id) as has_bio')
                                                    ->orderByDesc('has_bio')
                                                    ->orderBy('name'),
                                            )
                                            ->searchable()
                                            ...
                                            ->helperText(function ($state): ?string {
                                                ...
                                                $hasBio = UserBio::where('user_id', $state)->exists();

                                                if (! $hasBio) {
                                                    return 'This author doesn\'t have a bio yet — their posts won\'t display author information.';
                                                }
