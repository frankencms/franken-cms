...
class FeedService
{
    ...
    protected function buildRssFeed(): string
    {
        ...
        foreach ($posts as $post) {
            ...
            $xml .= '      <description><![CDATA[' . $this->escapeCdata($excerpt) . ']]></description>' . PHP_EOL;

            if ($includeFullText) {
                $content = $this->getPostContent($post);
                $xml .= '      <content:encoded><![CDATA[' . $this->escapeCdata($content) . ']]></content:encoded>' . PHP_EOL;
            }
    ...
    protected function buildAtomFeed(): string
    {
        ...
        foreach ($posts as $post) {
            ...
            $excerpt = $this->getExcerpt($post);
            $xml .= '    <summary><![CDATA[' . $this->escapeCdata($excerpt) . ']]></summary>' . PHP_EOL;

            if ($includeFullText) {
                $content = $this->getPostContent($post);
                $xml .= '    <content type="html"><![CDATA[' . $this->escapeCdata($content) . ']]></content>' . PHP_EOL;
            }
    ...
    /**
     * Escape CDATA-breaking sequences in content.
     *
     * The sequence ]]> terminates a CDATA block. If content contains this
     * sequence, it must be split to prevent XML injection. The standard
     * technique is to replace ]]> with ]]]]><![CDATA[> which closes the
     * current CDATA block after ]], starts a new one, and begins it with >.
     */
